 ...
 * -----------------
 * PT: Lógica de controle de comunicação. 
 *     Esta FSM gera os comandos JEDEC (ACT, PRE, RD/WR) com base no estado da página.
 * 
 * EN: Communication control logic.
 *     This FSM generates JEDEC commands (ACT, PRE, RD/WR) based on the page status.
 */
module comunication_control_logic(
    ...
    // PT: Lógica Combinacional de Saída | EN: Combinational Output Logic
    always @ (*) begin
        cpu_ready     = 1'b0;
        update_row_en = 1'b0;
        CS            = 1'b0;
        ...
        case (state)
            BOOT_WAIT, PRE_WAIT, ACT_WAIT, WAIT_CCD1, WAIT_CCD2, CDC_WAIT1, CDC_WAIT2: begin
                {RAS, CAS, WE} = 3'b111; // NOP
            end
            
            WAIT_CCD3: begin
                {RAS, CAS, WE} = 3'b111;
                cmd_ack = 1'b1; // PT: Pop da FIFO | EN: FIFO Pop signal
            end
            
            IDLE: begin
                {RAS, CAS, WE} = 3'b111; 
                // PT: Só pronto se a página estiver em estado conhecido
                // EN: Ready only if page is in a known state
    ...
    // PT: Transição de Estados | EN: State Transitions
