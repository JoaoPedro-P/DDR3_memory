 ...
 * module: tb_ddr_mem
 * -----------------
 * PT: Testbench do Sistema de Memória DDR3 com Clocks Independentes. 
 *     Simula uma CPU realizando operações massivas de escrita e leitura aleatória.
 *     Inclui cálculo automático de períodos e suporte a domínios de clock diferentes (CDC).
 * 
 * EN: DDR3 Memory System Testbench with Independent Clocks.
 *     Simulates a CPU performing massive random write and read operations.
 *     Includes automatic period calculation and support for different clock domains (CDC).
 */
`timescale 1ns/1ps
...
module tb_ddr_mem;
    ...
    // =========================================================================
    parameter FREQ_MEM = 100; // PT: Frequência da Memória (MHz) | EN: Memory frequency (MHz)
    parameter FREQ_CPU = 200; // PT: Frequência da CPU (MHz) | EN: CPU frequency (MHz)
    ...
    // 1. PT: Clock da Memória | EN: Memory Clock
    initial begin
        clk = 0;
        forever #(T_MEM / 2.0) clk = ~clk; 
    end

    // 2. PT: Clock DQS Defasado (90°) | EN: Phase-shifted DQS Clock (90°)
    initial begin
        clk_90 = 0;
        #(T_MEM / 4.0); // PT: 90° é 1/4 do período | EN: 90 degrees is 1/4 of the period
        forever #(T_MEM / 2.0) clk_90 = ~clk_90;
    end

    // 3. PT: Clock do Processador/AXI | EN: Processor/AXI Clock
    initial begin
        cpu_clk = 0;
    ...
    task write_burst(input [26:0] addr, input [63:0] data_in);
        begin
            // 1) PT: Solicita comando | EN: Request command
            cpu_req   = 1'b1;
            cpu_rnw   = 1'b0; // WRITE
            cpu_addr  = addr;
            cpu_wr_en = 1'b0; 
            
            // 2) PT: Aguarda handshake do controlador | EN: Wait for controller handshake
            @(posedge cpu_clk);
    ...
    task read_burst(input [26:0] addr, output [63:0] data_out);
        ...
        begin
            ...
            // 1) PT: Solicita leitura | EN: Request read
            cpu_req  = 1'b1;
            cpu_rnw  = 1'b1; // READ
            cpu_addr = addr;

            // 2) PT: Aguarda aceitação | EN: Wait for acceptance
            @(posedge cpu_clk);
            while (cpu_ready !== 1'b1) begin
                @(posedge cpu_clk);
            end
            ...
            // 3) PT: Aguarda dados na RX FIFO | EN: Wait for data in RX FIFO
	...
	cpu_wstrb = 0;
        ...
        for (i = 0; i < 100000; i = i + 1) begin
            ...
            test_col  = $urandom_range(0, 1023) & 10'h3F8; // PT: Alinhado a BL8 | EN: BL8 Aligned
