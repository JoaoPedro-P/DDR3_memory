...
// =========================================================================
// [PT] Modulo: tb_datapath
// [PT] Descrição: Testbench do datapath que avalia as operações de leitura e escrita através das FIFOs e barramento físico DDR3.
// [EN] Module: tb_datapath
// [EN] Description: Datapath testbench that evaluates read and write operations through FIFOs and physical DDR3 bus.
// =========================================================================

module tb_datapath();

    // -------------------------------------------------------------------------
    // [PT] Sinais Internos do Testbench / [EN] Testbench Internal Signals
    // -------------------------------------------------------------------------
    reg clk;
    reg clk_90;
    reg rst_n;
    ...
    // [PT] Sinais Físicos Bidirecionais (Tri-State) / [EN] Bidirectional Physical Signals (Tri-State)
    wire [7:0] DQ;
    wire DQS;
    wire DQS_n;

    // [PT] Controles do Mock da Memória DDR3 (Testbench) / [EN] DDR3 Memory Mock Controls (Testbench)
    reg [7:0] tb_dq;
    reg tb_dqs;
    reg tb_dqs_n;
    reg tb_drive_bus;
    ...
    // -------------------------------------------------------------------------
    // [PT] Roteamento Tri-State no Testbench / [EN] Tri-State Routing in Testbench
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // [PT] Geração de Clocks (NOVA RELAÇÃO DE FREQUÊNCIAS) / [EN] Clock Generation (NEW FREQUENCY RELATIONSHIP)
    // -------------------------------------------------------------------------
    // [PT] 1. Clock Principal da PHY (100 MHz -> Período 10ns) / [EN] 1. Main PHY Clock (100 MHz -> 10ns Period)
    initial begin
        clk = 0;
        forever #5 clk = ~clk;
    end

    // [PT] 2. Clock Atrasado em 90 graus (100 MHz, deslocado em 2.5ns) / [EN] 2. 90-degree Delayed Clock (100 MHz, shifted by 2.5ns)
    initial begin
        clk_90 = 0;
        #2.5; 
    ...
    // [PT] 3. Clock do Processador RISC-V (200 MHz -> Período 5ns) / [EN] 3. RISC-V Processor Clock (200 MHz -> 5ns Period)
    // [PT] A CPU agora é o dobro da velocidade da memória / [EN] CPU is now twice the speed of memory
    initial begin
        cpu_clk = 0;
    ...
    // -------------------------------------------------------------------------
    // [PT] Monitor de Escrita (DDR TX Auto-Checker) / [EN] Write Monitor (DDR TX Auto-Checker)
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // [PT] TASKS (Ações do Testbench) / [EN] TASKS (Testbench Actions)
    // -------------------------------------------------------------------------
    
    // [PT] Task 1: A CPU envia 4 palavras para a TX FIFO (Agora muito mais rápido) / [EN] Task 1: CPU sends 4 words to TX FIFO (Now much faster)
    ...
    // [PT] Task 2: Dispara a máquina de estados da PHY para consumir a FIFO / [EN] Task 2: Triggers the PHY state machine to consume the FIFO
    task phy_execute_write_burst(input integer op_num);
        begin
            ...
            // [PT] Aguarda a finalização do estado de controle / [EN] Waits for control state completion
    ...
    // [PT] Task 3: Simula a Memória DDR3 enviando dados para a FPGA (DDR RX) / [EN] Task 3: Simulates DDR3 Memory sending data to FPGA (DDR RX)
    task mem_execute_read(input integer op_num, input [7:0] msb, input [7:0] lsb);
        begin
            @(negedge clk);
            tb_drive_bus = 1;
            
            // [PT] Preamble da memória / [EN] Memory preamble
            tb_dqs = 0; tb_dqs_n = 1; 
            #10; 
            
            // [PT] Borda de Subida (Envia LSB) / [EN] Rising Edge (Sends LSB)
            tb_dqs = 1; tb_dqs_n = 0; tb_dq = lsb;
            #5;
            
            // [PT] Borda de Descida (Envia MSB) / [EN] Falling Edge (Sends MSB)
            ...
            // [PT] Espera a CPU rápida (200MHz) detectar a flag válida / [EN] Waits for the fast CPU (200MHz) to detect valid flag
            @(posedge cpu_clk);
            wait(rx_valid == 1'b1);
            ...
            // [PT] Espera o sinal baixar antes do próximo teste / [EN] Waits for the signal to drop before next test
            wait(rx_valid == 1'b0); 
            $display("---------------------------------------------------------------");
        end
    endtask
    ...
    // -------------------------------------------------------------------------
    // [PT] SEQUÊNCIA DE TESTES (10 Operações) / [EN] TEST SEQUENCE (10 Operations)
    // -------------------------------------------------------------------------
    initial begin
        $display("===============================================================");
